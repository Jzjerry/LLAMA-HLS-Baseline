...
void init_quantized_tensors(void **ptr,
                            QuantizedTensor<int8_t, SIZE, GROUP_SIZE> *tensor, // Use corrected type
                            ...
                            int group_size)  // Explicit group size parameter
{
    ...
    for (int i = 0; i < n; i++) {
        ...
        p = (int8_t *)p + size; // Advance pointer past q data
        // Check data
        // if (i == 0){
    ...
    void *ptr, uint8_t shared_classifier)
{
    ...
    if (shared_classifier) {
        ...
        std::memcpy(w->wcls, w->q_tokens, sizeof(QuantizedTensor<int8_t, vocab_size * dim, group_size_gs>));
    } else {
                       ...
                       vocab_size, seq_len, group_size_gs> *weights)
{
    ...
    std::cout << "Start Loading Checkpoint..." << std::endl;
    ...
    Tokenizer *tokenizer, Sampler *sampler, char *prompt, int steps, auto kernelpath)
{
  ...
  std::string xclbin_path =model_base_path+ "/hw_end_mp/forward.xclbin"; // Default for CSIM
