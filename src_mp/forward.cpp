...
LayerLoop:
    for (int l = 0; l < n_layers; ++l) {
        ...
        QuantizedTensor<xq_attn_t, dim, GS> xq_attn; // 重命名
        ...
        QuantizedTensor<xq_ffn_t, dim, GS> xq_ffn; // FFN 输入量化结果
        QuantizedTensor<hq_t, hidden_dim, GS> hq;
        ...
        quantize<xq_attn_t, dim, GS>(&xq_attn, xb_attn, GS); // 使用 l
        
        matmul<xq_attn_t, wq_t, dim, dim>(q, xq_attn.q, xq_attn.s, (w->wq + l)->q, (w->wq + l)->s); // 使用 l
        matmul<xq_attn_t, wk_t, dim, kv_dim_local>(k, xq_attn.q, xq_attn.s, (w->wk + l)->q, (w->wk + l)->s); // 使用 l
        matmul<xq_attn_t, wv_t, dim, kv_dim_local>(v, xq_attn.q, xq_attn.s, (w->wv + l)->q, (w->wv + l)->s); // 使用 l
        ...
        quantize<xq_attn_t, dim, GS>(&xq_attn, attn_out_unquant, GS); // 使用 l
        matmul<xq_attn_t, wo_t, dim, dim>(attn_output_no_residual, xq_attn.q, xq_attn.s, (w->wo + l)->q, (w->wo + l)->s); // 使用 l
    ...
    AddResidualAttn:
        ...
        quantize<xq_ffn_t, dim, GS>(&xq_ffn, xb_ffn, GS); // 使用 l
        matmul<xq_ffn_t, w1_t, dim, hidden_dim>(hb, xq_ffn.q, xq_ffn.s, (w_ffn->w1 + l)->q, (w_ffn->w1 + l)->s); // 使用 l
        matmul<xq_ffn_t, w3_t, dim, hidden_dim>(hb2, xq_ffn.q, xq_ffn.s, (w_ffn->w3 + l)->q, (w_ffn->w3 + l)->s); // 使用 l
        ...
        quantize<hq_t, hidden_dim, GS>(&hq, hb, GS); // 使用 l
        matmul<hq_t, w2_t, hidden_dim, dim>(ffn_output_no_residual, hq.q, hq.s, (w_ffn->w2 + l)->q, (w_ffn->w2 + l)->s); // 使用 l
...
#pragma HLS INTERFACE s_axilite port=return
    ...
    QuantizedTensor<xq_t, dim, GS> xq; // Use global GS
...
read_x_stream_final:
    ...
    quantize<xq_t, dim, GS>(&xq, x_norm, GS); // Pass dim and GS as template args

    // matmul call should be okay
    matmul<xq_t, wcls_t, dim, vocab_size>(logits_out, xq.q, xq.s, w->wcls->q, w->wcls->s);
}