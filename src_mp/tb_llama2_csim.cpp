    ...
    void *ptr, uint8_t shared_classifier)
{
    ...
    init_quantized_tensors<8, dim * dim, group_size_gs>         (&ptr, w->wq, n_layers, dim * dim, group_size_gs);
    init_quantized_tensors<8, kv_dim_calc * dim, group_size_gs> (&ptr, w->wk, n_layers, kv_dim_calc * dim, group_size_gs);
    init_quantized_tensors<8, kv_dim_calc * dim, group_size_gs> (&ptr, w->wv, n_layers, kv_dim_calc * dim, group_size_gs);
    init_quantized_tensors<8, dim * dim, group_size_gs>         (&ptr, w->wo, n_layers, dim * dim, group_size_gs);

    // Map FFN weights
    init_quantized_tensors<8, hidden_dim * dim, group_size_gs>  (&ptr, w->w1, n_layers, hidden_dim * dim, group_size_gs);
    init_quantized_tensors<8, dim * hidden_dim, group_size_gs>  (&ptr, w->w2, n_layers, dim * hidden_dim, group_size_gs);
    init_quantized_tensors<8, hidden_dim * dim, group_size_gs>  (&ptr, w->w3, n_layers, hidden_dim * dim, group_size_gs);
