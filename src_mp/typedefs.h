...
typedef ap_int<8> xq_attn_t;
typedef ap_int<8> xq_ffn_t;
typedef ap_int<8> hq_t;

typedef ap_int<8> xq_t;

// Weight Precisions
typedef ap_int<8> wq_t;
typedef ap_int<8> wk_t;
typedef ap_int<8> wv_t;
typedef ap_int<8> wo_t;

typedef ap_int<8> w1_t;
typedef ap_int<8> w2_t;
typedef ap_int<8> w3_t;
...
template <typename DT, int SIZE, int GROUP_SIZE>
...
struct RunState
{
    ...
    // QuantizedTensor<int8_t, dim, GS> xq;        // quantized x (dim,)
    // QuantizedTensor<int8_t, hidden_dim, GS> hq; // quantized hb (hidden_dim,)
